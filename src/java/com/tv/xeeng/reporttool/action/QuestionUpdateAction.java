	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (!loggedInUser.getIsAdmin()
			...
			if(isValidate == true) {
				...
				qBean.setQuestionId(_qForm.getQuestionId());
				...
				int rs =  new QuestionDAO().updateAQuestion(qBean);
				if(rs > 0) {
					request.setAttribute("msg_notifi", "Đã cập nhật thành công!");
					return mapping.findForward("success");
				} else {
					request.setAttribute("msg_notifi", "Cập nhật thất bại <br/> Bạn hãy kiểm tra lại các thông tin!");
					return mapping.findForward("fail");
				}
			} else {
				request.setAttribute("msg_notifi", "Cập nhật thất bại <br/> Bạn hãy kiểm tra lại các thông tin đầu vào!");
				return mapping.findForward("fail");
			}
