...
public class UpdateGoldenHourAction extends Action {
	 private static Logger logger = Logger.getLogger(UpdateGoldenHourAction.class);
	@Override
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(Integer.parseInt(_gdhForm.getTminutes()) < 10) {
			String tM = _gdhForm.getTminutes();
			_gdhForm.setTminutes("0"+ tM);
		}
		if(Integer.parseInt(_gdhForm.getTsecond()) < 10) {
			String tS = _gdhForm.getTsecond();
			_gdhForm.setTsecond("0"+ tS);
		}
		...
		if(isValidate == true) {
			...
			int rs = new GoldenhourDAO().updatGoldenHour(gdhBean);
			if(rs > 0) {
				request.setAttribute("msg_notifi", "Đã cập nhật giờ vàng thành công");
